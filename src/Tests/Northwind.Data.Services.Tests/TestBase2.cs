...
namespace Northwind.Data.Services.Tests
{
    ...
    public abstract class TestBase2
    {
        ...
        protected TestBase2(params Assembly[] serviceAssemblies)
            : this(null, serviceAssemblies) { }

        protected TestBase2(string serviceClientBaseUri, params Assembly[] serviceAssemblies)
        {
            ...
            EndpointHost.ConfigureHost(this.AppHost, "TestBase2", serviceManager);
        ...
        public object ExecutePath(string httpMethod, string pathInfo, bool secureRequest = false)
        {
            var urlParts = new UrlParts(pathInfo);
            return ExecutePath(httpMethod, urlParts.PathInfo, urlParts.QueryString, null, null, secureRequest);
        }

        private TResponse ExecutePath<TResponse>(string httpMethod, UrlParts urlParts, object requestDto, bool secureRequest = false)
        {
            return (TResponse)ExecutePath(httpMethod, urlParts.PathInfo, urlParts.QueryString, null, requestDto, secureRequest);
        }

        public TResponse ExecutePath<TResponse>(string httpMethod, string pathInfo, object requestDto, bool secureRequest = false)
        {
            var urlParts = new UrlParts(pathInfo);
            return (TResponse)ExecutePath(httpMethod, urlParts.PathInfo, urlParts.QueryString, null, requestDto, secureRequest);
        }
            ...
            bool secureRequest = false)
        {
            ...
            return ExecutePath(httpMethod, pathInfo, queryString, formData, json, secureRequest);
        }
            ...
            bool secureRequest = false)
        {
            ...
            var svc = httpReq.Container.TryResolve<AuthService>();
            var authService = new AuthService() { RequestContext = new MockRequestContext() };
            //authService.SetResolver(this.AppHost);
            //this.AppHost.Register(authService);
            httpReq.Container.Register(authService);

            var httpRes2 = new MockHttpResponse();

            // Assume it needs authentication for tests
            if(secureRequest)
                httpReq.Headers.Add(HttpHeaders.Authorization, "Basic " + Convert.ToBase64String(Encoding.UTF8.GetBytes(string.Concat("admin", ":", "admin"))));
            
            var request = httpHandler.CreateRequest(httpReq, httpHandler.RequestName);
            var response = httpHandler.GetResponse(httpReq, httpRes2, request);
        ...
        {
            private readonly TestBase2 parent;
            ServiceManager ServiceManager { get; set; }

            public DirectServiceClient(TestBase2 parent, ServiceManager serviceManager)
            {
                this.parent = parent;

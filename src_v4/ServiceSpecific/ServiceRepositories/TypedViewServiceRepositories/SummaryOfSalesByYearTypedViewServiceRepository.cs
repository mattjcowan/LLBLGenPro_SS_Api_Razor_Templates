...
{ 
    public partial class SummaryOfSalesByYearTypedViewServiceRepository : TypedViewServiceRepositoryBase<SummaryOfSalesByYear>, ISummaryOfSalesByYearTypedViewServiceRepository
	...
	// __LLBLGENPRO_USER_CODE_REGION_END 
    {
        ...
        partial void OnAfterFetchSummaryOfSalesByYearQueryCollectionRequest(IDataAccessAdapter adapter, SummaryOfSalesByYearQueryCollectionRequest request, SummaryOfSalesByYearTypedView typedView, SortExpression sortExpression, string[] includedFieldNames, IRelationPredicateBucket predicateBucket, int pageNumber, int pageSize, int limit, int totalItemCount);
        #endregion
        ...
        public SummaryOfSalesByYearTypedViewServiceRepository()
        {
            OnCreateRepository();
        }

        // Description for parameters: http://datatables.net/usage/server-side
        public DataTableResponse GetDataTableResponse(SummaryOfSalesByYearDataTableRequest request)
        {
            ...
            var entities = Fetch(new SummaryOfSalesByYearQueryCollectionRequest
                {
                    Filter = filter, 
        ...
        public SummaryOfSalesByYearCollectionResponse Fetch(SummaryOfSalesByYearQueryCollectionRequest request)
        {
            ...
            var typedView = new SummaryOfSalesByYearTypedView();
            using (var adapter = DataAccessAdapterFactory.NewDataAccessAdapter())
            {
                OnBeforeFetchSummaryOfSalesByYearQueryCollectionRequest(adapter, request, sortExpression, includedFieldNames, predicateBucket,
                ...
                OnAfterFetchSummaryOfSalesByYearQueryCollectionRequest(adapter, request, typedView, sortExpression, includedFieldNames, predicateBucket,
                    request.PageNumber, request.PageSize, request.Limit, totalItemCount);
            }

            var dtos = new SummaryOfSalesByYearCollection();
            ...
            var response = new SummaryOfSalesByYearCollectionResponse(dtos, request.PageNumber,
                                                          request.PageSize, totalItemCount);
            return response;       
        }

        private SummaryOfSalesByYear Map(SummaryOfSalesByYearRow row, string[] fieldNames)
        {
            var hasFn = fieldNames != null && fieldNames.Any();
            var item = new SummaryOfSalesByYear();

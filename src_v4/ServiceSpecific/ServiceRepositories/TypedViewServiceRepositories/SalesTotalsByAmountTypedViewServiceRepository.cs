...
{ 
    public partial class SalesTotalsByAmountTypedViewServiceRepository : TypedViewServiceRepositoryBase<SalesTotalsByAmount>, ISalesTotalsByAmountTypedViewServiceRepository
	// __LLBLGENPRO_USER_CODE_REGION_START SsSvcAdditionalInterfaces 
	// __LLBLGENPRO_USER_CODE_REGION_END 
    {
        #region Class Extensibility Methods
        partial void OnCreateRepository();
        partial void OnBeforeFetchSalesTotalsByAmountQueryCollectionRequest(IDataAccessAdapter adapter, SalesTotalsByAmountQueryCollectionRequest request, SortExpression sortExpression, string[] includedFieldNames, IRelationPredicateBucket predicateBucket, int pageNumber, int pageSize, int limit);
        partial void OnAfterFetchSalesTotalsByAmountQueryCollectionRequest(IDataAccessAdapter adapter, SalesTotalsByAmountQueryCollectionRequest request, SalesTotalsByAmountTypedView typedView, SortExpression sortExpression, string[] includedFieldNames, IRelationPredicateBucket predicateBucket, int pageNumber, int pageSize, int limit, int totalItemCount);
        #endregion
        ...
        public SalesTotalsByAmountTypedViewServiceRepository()
        {
            OnCreateRepository();
        }

        // Description for parameters: http://datatables.net/usage/server-side
        public DataTableResponse GetDataTableResponse(SalesTotalsByAmountDataTableRequest request)
        {
            ...
            var entities = Fetch(new SalesTotalsByAmountQueryCollectionRequest
                {
                    Filter = filter, 
        ...
        public SalesTotalsByAmountCollectionResponse Fetch(SalesTotalsByAmountQueryCollectionRequest request)
        {
            ...
            var typedView = new SalesTotalsByAmountTypedView();
            using (var adapter = DataAccessAdapterFactory.NewDataAccessAdapter())
            {
                OnBeforeFetchSalesTotalsByAmountQueryCollectionRequest(adapter, request, sortExpression, includedFieldNames, predicateBucket,
                ...
                OnAfterFetchSalesTotalsByAmountQueryCollectionRequest(adapter, request, typedView, sortExpression, includedFieldNames, predicateBucket,
                    request.PageNumber, request.PageSize, request.Limit, totalItemCount);
            }

            var dtos = new SalesTotalsByAmountCollection();
            ...
            var response = new SalesTotalsByAmountCollectionResponse(dtos, request.PageNumber,
                                                          request.PageSize, totalItemCount);
            return response;       
        }

        private SalesTotalsByAmount Map(SalesTotalsByAmountRow row, string[] fieldNames)
        {
            var hasFn = fieldNames != null && fieldNames.Any();
            var item = new SalesTotalsByAmount();
            if (!hasFn || fieldNames.Contains("SaleAmount", StringComparer.OrdinalIgnoreCase))
                item.SaleAmount = row.SaleAmount;
